...
export function ChapterSection({ chapter }: { chapter: Chapter }) {
  ...
  const pointerEvents = state.activeChapterIndex === chapter.index ? "auto" : "none";
  ...
  return (
    <motion.section 
      ...
      className="absolute inset-0 flex items-center justify-center py-32 px-6 md:px-12 lg:px-24 pointer-events-none"
      ...
      >
        <div className="space-y-16">
          ...
          <div className="space-y-4">
            ...
            <motion.p 
              className="text-xl md:text-2xl text-white/60 font-sans"
            >
              {chapter.subtitle}
            </motion.p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-16">
            ...
            <div className="md:col-span-5 space-y-12">
              <motion.blockquote 
                className={`text-2xl md:text-3xl leading-snug text-white/90 border-l-2 pl-6 ${typoClass}`}
              ...
              {chapter.anchor && (
                <motion.div
                  className="p-6 rounded-lg bg-white/[0.02] border border-white/5 backdrop-blur-sm"
                >
                  <h4 className="text-xs font-mono text-white/40 uppercase tracking-widest mb-2">Anchor Object</h4>
                  <div className="text-white/80 font-medium mb-1">{chapter.anchor.object}</div>
                  <div className="text-white/50 text-sm italic">{chapter.anchor.meaning}</div>
                </motion.div>
              )}
            </div>
            ...
            <div className="md:col-span-7">
              <motion.p
                className={`text-lg md:text-xl text-white/70 leading-relaxed ${typoClass}`}
              >
                {chapter.narrative}
              </motion.p>
            </div>
